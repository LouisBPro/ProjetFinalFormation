  ...
  ngOnInit(): void {
    ...
    panier.forEach((value: number, key: string) => {
      ...
      if (!this.restaurantsIds.includes(restaurantId)) {
        ...
        this.panierParRestaurant[this.restaurantsIds.length - 1] = new Map<string,number>();
      }
    ...
    this.panierParRestaurant.forEach((panier : Map<string, number>) => {
      ...
      this.restaurantService.getById(this.restaurantsIds[index]).subscribe((restaurant) => {
        this.validerService.enregistrer(restaurant, prixTotal as unknown as string, panier).subscribe((ok)=>{
